...
namespace BatteryHealth
{
    ...
    public static class BatteryInfo
    {
        ...
        public static BatteryInformation GetBatteryInformation()
        {
            ...
            try
            {
                ...
                return new BatteryInformation()
                {
                    ...
                    BatteryState = GetBatteryState(updatedStatus.PowerState),
                    BatteryHealthPercentage = GetBatteryHealthPercentage(updatedBatteryInformation.FullChargedCapacity, updatedBatteryInformation.DesignedCapacity)
                };
        ...
        private static string GetBatteryState(Win32.POWER_STATE powerState)
        {
            if ((powerState & Win32.POWER_STATE.BATTERY_POWER_ONLINE) != 0)
                return "Online";
            else if ((powerState & Win32.POWER_STATE.BATTERY_DISCHARGING) != 0)
                return "Discharging";
            else if ((powerState & Win32.POWER_STATE.BATTERY_CHARGING) != 0)
                return "Charging";
            else if ((powerState & Win32.POWER_STATE.BATTERY_CRITICAL) != 0)
                return "Critical";
            else
                return "Unknown";
        }

        private static double GetBatteryHealthPercentage(int currentCapacity, int designedMaxCapacity)
        {
            double percentage = (double)currentCapacity / designedMaxCapacity * 100;
